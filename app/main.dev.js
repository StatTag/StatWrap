...
// Track enhanced search service state in the main process
let enhancedSearchIndexingQueue = [];
let currentlyIndexingProject = null;
let searchIndexingStats = {
  totalDocuments: 0,
  documentsByType: {},
  lastUpdate: Date.now(),
  performance: {
...
let backendDocumentStore = new Map();
let searchConfig = null;

/**
 * Initialize backend search configuration
 */
  ...
  try {
    ...
    searchConfig = {
      ...
      excludeDirectories: [
        ...
        'target', 'bin', 'obj', '.statwrap'
      ]
    };

    console.log('Backend search configuration initialized');
  } catch (error) {
    console.error('Error initializing backend search configuration:', error);
  }
}

/**
 * Handle request to get enhanced search statistics
 */
ipcMain.on('statwrap-enhanced-search-stats-request', async (event) => {
  const response = {
    stats: searchIndexingStats,
    status: {
      inProgress: currentlyIndexingProject !== null,
      projectId: currentlyIndexingProject,
      queueLength: enhancedSearchIndexingQueue.length
    },
    error: false,
    errorMessage: ''
  };

  event.sender.send('statwrap-enhanced-search-stats-response', response);
});

/**
 * Handle request to index a specific project with enhanced search
 */
ipcMain.on('statwrap-enhanced-search-index-project-request', async (event, projectId) => {
  const response = {
    status: {
      inProgress: currentlyIndexingProject !== null,
      projectId: currentlyIndexingProject,
  ...
  try {
    if (projectId &&
      !enhancedSearchIndexingQueue.includes(projectId) &&
      currentlyIndexingProject !== projectId) {
      enhancedSearchIndexingQueue.push(projectId);

      if (currentlyIndexingProject === null) {
        processNextEnhancedSearchIndexing();
      }
    }
  } catch (error) {
    response.error = true;
    response.errorMessage = `Error scheduling enhanced project indexing: ${error.message}`;
  }

  event.sender.send('statwrap-enhanced-search-index-project-response', response);
});

/**
 * Handle request to index all projects with enhanced search
 */
ipcMain.on('statwrap-enhanced-search-index-all-request', async (event) => {
  const response = {
    status: {
      inProgress: currentlyIndexingProject !== null,
      projectId: currentlyIndexingProject,
  ...
  try {
    const userDataPath = app.getPath('userData');
    const projectListPath = path.join(userDataPath, 'projects.json'); 

    if (fs.existsSync(projectListPath)) {
      const projectsData = JSON.parse(fs.readFileSync(projectListPath, 'utf8'));
      const projects = projectsData.projects || projectsData || [];

      projects.forEach(project => {
        if (!enhancedSearchIndexingQueue.includes(project.id) &&
          currentlyIndexingProject !== project.id) {
          enhancedSearchIndexingQueue.push(project.id);
        }
      });
    }
    ...
    response.status.queueLength = enhancedSearchIndexingQueue.length;
  } catch (error) {
    response.error = true;
    response.errorMessage = `Error scheduling all projects for enhanced indexing: ${error.message}`;
  }

  event.sender.send('statwrap-enhanced-search-index-all-response', response);
});

/**
 * Handle enhanced search request - delegate to renderer process
 */
ipcMain.on('statwrap-enhanced-search-request', async (event, searchParams) => {
  ...
  const response = {
    ...
    error: false,
    errorMessage: '',
    message: 'Search delegated to renderer process with ElasticLunr'
  };

  try {
    if (!query || typeof query !== 'string' || query.trim() === '') {
      response.searchTime = Date.now() - searchStartTime;
      event.sender.send('statwrap-enhanced-search-response', response);
      return;
    }

    // For now, just return empty results and let renderer handle the search
    response.searchTime = Date.now() - searchStartTime;
    response.message = 'Search should be handled by renderer process ElasticSearchService';

    // Update performance metrics
    searchIndexingStats.performance.totalSearches++;
    searchIndexingStats.performance.averageSearchTime =
      (searchIndexingStats.performance.averageSearchTime * (searchIndexingStats.performance.totalSearches - 1) + response.searchTime)
      / searchIndexingStats.performance.totalSearches;
...
/**
 * Handle request to show item in folder
 */
ipcMain.on('show-item-in-folder', (event, itemPath) => {
  try {
    if (fs.existsSync(itemPath)) {
      ...
      console.warn('Enhanced Search: Item path does not exist:', itemPath);
    }
  } catch (error) {
    console.error('Enhanced Search: Error showing item in folder:', error);
  }
});

/**
 * Handle request to open file with default application
 */
ipcMain.on('open-file-with-default', (event, filePath) => {
  try {
    if (fs.existsSync(filePath)) {
      shell.openPath(filePath);
    } else {
      console.warn('Enhanced Search: File path does not exist:', filePath);
    }
  } catch (error) {
    console.error('Enhanced Search: Error opening file:', error);
  }
});

/**
 * Process the next project in the enhanced search indexing queue
 */
async function processNextEnhancedSearchIndexing() {
  if (enhancedSearchIndexingQueue.length === 0) {
    currentlyIndexingProject = null;
    return;
  }

  const projectId = enhancedSearchIndexingQueue.shift();
  currentlyIndexingProject = projectId;
  ...
  try {
    ...
    if (global.mainWindow) {
      global.mainWindow.webContents.send('statwrap-enhanced-search-status-response', {
        status: {
          inProgress: true,
          projectId,
          queueLength: enhancedSearchIndexingQueue.length
    ...
    await indexProjectForEnhancedSearch(project);
    updateSearchIndexingStats();

    // Waiting a bit to prevent UI blocking
    ...
    console.error('Enhanced Search: Error during indexing:', error);
    currentlyIndexingProject = null;
    processNextEnhancedSearchIndexing();
  }
}
...
/**
 * Index a project for enhanced search (basic file system analysis)
 */
async function indexProjectForEnhancedSearch(project) {
  ...
  try {
    ...
    const metadataPath = path.join(project.path, '.statwrap', 'metadata.json');
    let metadata = {};
    ...
    if (fs.existsSync(metadataPath)) {
      try {
        ...
        console.log(`Enhanced Search: Error reading metadata for ${project.id}:`, error.message);
      }
    }

    // Count documents in project
    documentCount += 1; // Project itself

    // Count people
    if (metadata.people && Array.isArray(metadata.people)) {
      documentCount += metadata.people.length;
    }
    ...
    if (metadata.assets) {
      documentCount += countAssets(metadata.assets);
    }

    if (metadata.externalAssets) {
      documentCount += countAssets(metadata.externalAssets);
    }

    // Count notes
    let noteCount = 0;
    if (metadata.notes && Array.isArray(metadata.notes)) {
      noteCount += metadata.notes.length;
    }
    if (metadata.people && Array.isArray(metadata.people)) {
    ...
    const fileCounts = await countFilesAndFolders(project.path);
    documentCount += fileCounts.files + fileCounts.folders;

    // Update performance stats
    const indexingTime = Date.now() - indexingStartTime;
    searchIndexingStats.performance.totalIndexingTime += indexingTime;
    searchIndexingStats.performance.documentsIndexed += documentCount;
    searchIndexingStats.totalDocuments += documentCount;

    // Update document counts by type
    searchIndexingStats.documentsByType.project = (searchIndexingStats.documentsByType.project || 0) + 1;
    searchIndexingStats.documentsByType.person = (searchIndexingStats.documentsByType.person || 0) + (metadata.people ? metadata.people.length : 0);
    searchIndexingStats.documentsByType.asset = (searchIndexingStats.documentsByType.asset || 0) + countAssets(metadata.assets) + countAssets(metadata.externalAssets);
    searchIndexingStats.documentsByType.note = (searchIndexingStats.documentsByType.note || 0) + noteCount;
    searchIndexingStats.documentsByType.file = (searchIndexingStats.documentsByType.file || 0) + fileCounts.files;
    searchIndexingStats.documentsByType.folder = (searchIndexingStats.documentsByType.folder || 0) + fileCounts.folders;

    console.log(`Enhanced Search: Analyzed project ${project.id} with ${documentCount} documents in ${indexingTime}ms`);

  } catch (error) {
    console.error(`Enhanced Search: Error analyzing project ${project.id}:`, error);
  }
}
 ...
 */
async function countFilesAndFolders(dirPath, currentCounts = { files: 0, folders: 0 }) {
  if (!searchConfig) return currentCounts;
    ...
    for (const item of items) {
      ...
      try {
        ...
        if (stats.isDirectory()) {
          if (searchConfig.excludeDirectories.includes(item)) {
            continue;
          }

          currentCounts.folders += 1;
          await countFilesAndFolders(itemPath, currentCounts);
        } else {
          currentCounts.files += 1;
        }
      } catch (itemError) {
        continue;
      }
...
/**
 * Update search indexing statistics
 */
function updateSearchIndexingStats() {
  searchIndexingStats.lastUpdate = Date.now();

  // Broadcast updated stats to renderer if needed
  if (global.mainWindow) {
    global.mainWindow.webContents.send('statwrap-enhanced-search-stats-response', {
      stats: searchIndexingStats,
      status: {
        inProgress: currentlyIndexingProject !== null,
        projectId: currentlyIndexingProject,
...
/**
 * Handle request to get enhanced search status
 */
ipcMain.on('statwrap-enhanced-search-status-request', async (event) => {
  const response = {
    status: {
      inProgress: currentlyIndexingProject !== null,
      projectId: currentlyIndexingProject,
  ...
  event.sender.send('statwrap-enhanced-search-status-response', response);
});
initializeBackendSearch();
export {
  enhancedSearchIndexingQueue,
  currentlyIndexingProject,
  searchIndexingStats,
  processNextEnhancedSearchIndexing,
  updateSearchIndexingStats,
  backendDocumentStore
};