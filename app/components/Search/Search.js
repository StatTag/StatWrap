      ...
      <SearchHeader>
        ...
        {searchMode !== 'basic' && !isInitializing && (
          <Alert severity="info" sx={{ mb: 2 }}>
            ...
            {searchMode === 'advanced' && (
              <>
                <strong>Advanced Search:</strong> Enhanced relevance scoring with fuzzy matching
              </>
            )}
          </Alert>
        )}
        ...
        {searchStats && !isInitializing && (
          <Box display="flex" alignItems="center" gap={2} mt={1}>
            ...
            <Chip
              label={`${searchStats.contentIndexedFiles || 0} files with content`}
              size="small"
              variant="outlined"
...
// Result item component for MiniSearch
const ResultItemComponent = ({ result, searchTerm, expanded, onToggleExpand, highlightText, showRelevanceScores }) => {
  ...
  const relevance = Math.floor((score || 0));
      ...
      case 'project':
        return (
          <Box>
            ...
            <Box mt={1}>
              <Link to={`${routes.PROJECT}/${item.projectId || item.id}`} style={{ color: 'inherit', textDecoration: 'none' }}>
            ...
            <Button
              onClick={() => {
                if (searchService.getSearchStats) {
                  const stats = searchService.getSearchStats();
                  console.log('MiniSearch Debug Info:', stats);
                  console.log('Document Store Size:', searchService.documentStore?.size || 0);

                  alert(`MiniSearch Statistics:
