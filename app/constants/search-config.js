...
// Extract extensions from existing content types for indexing
...
module.exports = {
  // ElasticLunr specific settings
  elasticLunr: {
    // Field configuration with boost values
    fields: {
      title: { boost: 10, store: true },
      content: { boost: 1, store: true },
      type: { boost: 5, store: true },
      projectName: { boost: 3, store: true },
      path: { boost: 2, store: true },
      tags: { boost: 8, store: true },
      metadata: { boost: 4, store: true }
    },
  ...
  indexing: {
    ...
    // Use existing content type extensions plus additional ones
  ...
  search: {
    ...
    // ElasticLunr specific search modes
    searchModes: {
      basic: {
        bool: 'OR',
        expand: true,
  ...
  contentTypeMapping: {
    getSearchCategory: (extension) => {
      ...
      if (contentType) {
        return contentType.categories[0]; // Usng first category
      }
