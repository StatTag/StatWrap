...
// TypeScript file extensions that we will scan.
// All lookups should be lowercase - we will do lowercase conversion before comparison.
const FILE_EXTENSION_LIST = ['ts', 'tsx', 'mts', 'cts'];

export default class TypeScriptHandler extends BaseCodeHandler {
  static id = 'StatWrap.TypeScriptHandler';

  constructor() {
    super(TypeScriptHandler.id, FILE_EXTENSION_LIST);
  }

  id() {
    return TypeScriptHandler.id;
  }
  ...
  getLibraries(uri, text) {
    ...
    // Also handles TypeScript-specific: import type { x } from 'module'
    const esImportMatches = [
      ...text.matchAll(/^[^/]*?import\s+(?:type\s+)?(?:(\*\s+as\s+\w+|\{[^}]*\}|\w+))\s+from\s+(['"]{1,}[\s\S]+?['"]{1,})/gim),
    ];
